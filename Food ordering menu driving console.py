...
list_hotels = []                 #Variable List of hotels.
...
def def_default():
    global list_drinks, list_foods, list_hotels, list_item_order, list_item_price
...
def def_main():
    while True:                                            #Repeat Menu until stops.
        ...
        if (len(input_1) == 1):                                           #Checking input length.
            ...
            elif (input_1 == 'S'):                                        #If input is "P".
                print("\n" * 10)                                         #Create 100 empty lines.
                def_swigy_driver()                                             #Start Payment function.
                break                                                     #Stop repeating Main Menu.
...
def def_order_menu():                                                                               #yousef
    while True:                                             # While looping to keep menu alive
        ...
        input_1 = str(input("Please Select Your Operation: ")).upper() # Options Handling : H-M-E.
        if len(input_1) == 1:
            if (input_1 == 'H'):  #Easy Access Checking Logic
                print("\n" * 10)
                def_order_hotel() # Show hotel Menu
                break
            elif (input_1 == 'M'):
                print("\n" * 10)
...
def def_full_file_reader():                                                                         #mustafa
    ...
    file_hotels = open('files'+navigator_symbol+'list_hotels.fsd', 'r') # Reading Services
    for i in file_hotels:
        list_hotels.append(str(i.strip()))
    file_hotels.close()
    ...
    i = 0
    while i <= (len(list_hotels) - 1):
        if list_hotels[i]:
            list_hotels[i] = str(list_hotels[i])
        i += 1
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    #list_services = sorted(list_hotels)
...
def def_order_hotel():
    while True:
        print("*" * 26 + "SELECT HOTEL" + "*" * 26)
        print(" |NO| |HOTEL NAME|")
        i=0
        while i<len(list_hotels):
            var_space = 1
            if i<=8:
                var_space=2

            if i< len(list_hotels):
                hotel = " (" + str(i + 1) + ")" + " " * var_space + str(list_hotels[i])
            else:
                hotel = " "
            print(hotel)
            i += 1
        print("\n (F) FOODS AND DRINKS                   (E) EXIT\n" + "_" * 72)

        input_1 = input("Please Select Your Operation: ").upper()  # Handling Menu Selection
        if (input_1 == 'F'):
            print("\n" * 10)
            def_food_drink_order() # Return to main menu by calling it out
            break
        if (input_1 == 'E'):
    ...
    while True:
            ...
            try:  # Cautions Error Handling to prevent program crashing and hand out exceptions as a readable error to notify user
                ...
                if ((int(input_1) <= len(list_foods) and int(input_1) > 0) or (
                    ...
                    try:
                        print("\n" + "_" * 72 + "\n" + str(list_foods[
                                                               int(input_1) - 1]))  # Handling Food Selection / The try/Execpt to handle out of index error as if it  not exists in the array
                    except:
                        pass
                    try:
                        print("\n" + "_" * 72 + "\n" + str(list_drinks[
                                                               int(input_1) - 21]))  # Handling Drinks Selection / The try/Execpt to handle out of index error as if it  not exists in the array
                    except:
                        pass
...
def def_swigy_driver():
    while True:
        print("*" * 32 + "SWIGGY DRIVER" + "*" * 33 + "\n") # Header & Styling
        print(" Baklava             RM 45.50  x  1\n___________________________________\nTOTAL PRICES:       RM 45.5")
        print("Baklava             RM 45.50  x  1\nFattoush            RM 51.80  x  3\nShish tawook        RM 39.50  x  5\n-----------------------------------\nDISCOUNT RATES:      % 5.0\nDISCOUNT AMOUNTS:   RM 18.92\n___________________________________\nTOTAL PRICES:       RM 378.48")
        print("Baklava             RM 45.50  x  1\nFattoush            RM 51.80  x  3\n-----------------------------------\nDISCOUNT RATES:      % 5.0\nDISCOUNT AMOUNTS:   RM 9.54\n___________________________________\nTOTAL PRICES:       RM 190.85")
        print("\n\nDid you want to take the order to deliever(Y/N)")



        print("\n (1) ORDER 1           (2) ORDER 2            (3) ORDER 3          (E) EXIT\n" + "_" * 72)
        input_1 = str(input("Please Select Your Operation: ")).upper()
        if (input_1 == '1'):
            print("\n" * 2)
